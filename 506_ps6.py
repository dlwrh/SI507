...
def get_airport(iatacode):
	page = requests.get( 'http://services.faa.gov/airport/status/'+iatacode , params = {'format':'json'})
	return page.json()
...
def extract_airport_data(iatacode):
	airportinfo = get_airport(iatacode)
	return airportinfo["IATA"], airportinfo["status"]["reason"], airportinfo["weather"]["temp"], airportinfo["weather"]["meta"]["updated"]
...
for el in fav_airports:
	print extract_airport_data(el)
...
for el in possible_airports:
	try:
		print extract_airport_data(el)
	except:
		print "Failed for Airport {}".format(el)
...
for el in possible_airports:
	try:
		outfile.write('{},{},{},{},\n'.format(*extract_airport_data(el)))
	except:
		print "Failed for Airport {}".format(el)
